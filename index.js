...
module.exports = function(options) {
  ...
  io.on('connection', function(socket) {
    ...
    var _user = JSON.parse(req.session.uinfo)
    ...
    var messageListener = wirerouter()
      .on(wire.DeviceLogMessage, function(channel, message) {
        if((message['serial'] && _user.type==1 && _mydevices.indexOf(message.serial) == -1)||_user.type==0){
          return;
        }
      ...
      .on(wire.JoinGroupByAdbFingerprintMessage, function(channel, message) {
        if(message['serial'] && !(_user.type==2 || _mydevices.indexOf(message.serial) > -1)){
          return;
        }
